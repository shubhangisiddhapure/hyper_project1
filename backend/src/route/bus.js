...
//get searched bus
router.get('/bus',[
    check('startCity','please enter start city').not().isEmpty(),
    check('endCity','please enter end city').not().isEmpty(),
    ...
    try
    {
        ...
        const buses = await Bus.find({startCity,endCity,departureDate:{"$gte":new Date(departureDate)}})
        console.log(buses)
        if(buses.length===0)
        {
